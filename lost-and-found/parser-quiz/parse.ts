...
const transformQuestion = (data: any): Question => {
    ...
    if (returning.blockId) {
        if (typeof parsed.db_type === 'undefined')
            throw `faltando de-para para o tipo ${
                data['Tipo']
            } em ${JSON.stringify(returning)}`
        ...
        if (parsed.db_type === 'auto_change_questionnaire') {
            ...
            ['onlychoice', 'multiplechoices', 'yesno', 'yesnomaybe'].includes(
                parsed.db_type
            )
        ) {
            for (const line of returning.optionsPath.split('\n')) {
                const ret = (
                    ['yesno', 'yesnomaybe'].includes(parsed.db_type)
                        ? yesnoRegex
                        : mcRegexp
                ).exec(line)

                if (!ret || !ret[1] || !ret[2])
                    throw `falha ao parsear linha ${line} em ${JSON.stringify(
                        returning
                    )}`
                ...
                for (const path of paths) {
                    if (path.toLowerCase().startsWith('p')) {
                        if (pergunta)
                            throw `só pode seguir pra uma pergunta por vez, ${line} em ${JSON.stringify(
                                returning
                            )}`
                        pergunta = path
                ...
                if (!pergunta)
                    throw `faltando pergunta, ${line} em ${JSON.stringify(
                        returning
                    )}`
...
const transformTask = (data: any): Task => {
    const str = data['ID Resposta']
        ? (data['ID Resposta'] as string).replace(/\s*ou\s*/, ',')
        : ''
...
const generateTarefasSql = (tasks: Task[], blocks: Block[]) => {
    ...
    for (let task of tasks) {
        ...
        const blocoAgrupador =
            blocks.find((r) => r.id === agrupador)?.description || agrupador
    ...
    for (let block of blocks) {
        ...
        for (let qc of quiz) {
            if (qc.questionnaire_id == block.db_id) {
                ...
                let escapedOptions =
                    qc.options !== null
                        ? escapeString(JSON.stringify(qc.options))
                        : null
                ...
                sqlStr += `INSERT INTO quiz_config(status, sort, type, code, question, questionnaire_id, intro, relevance, button_label, options, tarefas, change_to_questionnaire_id, tag)
                        VALUES ('${qc.status}', ${qc.sort}, '${qc.type}', '${
                    qc.code
                }', E'${escapedQuestion}',
                        ${
                            qc.questionnaire_id
                        }, E'${escapedIntro}', E'${escapedRelevance}', ${
                    qc.button_label === null ? 'null' : `'${qc.button_label}'`
                        ...
                        ${
                            escapedOptions === null
                                ? 'null'
                                : `E'${escapedOptions}'`
                        }, '${JSON.stringify(qc.tarefas)}', ${
                    qc.change_to_questionnaire_id
                }, '${JSON.stringify(qc.tag)}');\n`
            }
        }
    }
...
function boostrap() {
    ...
    for (const q of questions) {
        ...
        if (!row.questionnaire_id)
            throw `Faltando ID para o bloco ${q.blockId} -- pergunta ${q.questionId}`

        if (row.type === 'botao_fim') {
            row.button_label = bf_label
        } else if (
            row.type === 'onlychoice' ||
            row.type === 'multiplechoices'
        ) {
            row.options = []
            ...
            for (const option of q2.parsedType.options) {
                if (option.proxima_pergunta == q.questionId) {
                    if (q2.parsedType.db_type === 'multiplechoices') {
                        relevances.push(
                            `is_json_member('${option.opcao_clean}', ${q2.blockId}_${q2.questionId}_json)`
                        )
                    } else {
                        relevances.push(
                            `${q2.blockId}_${q2.questionId} == '${option.opcao_clean}'`
                        )
                    }
                }
            }
